...
import { ChevronDownIcon, CrossIcon, HamburgerMenuIcon } from '@govtechmy/myds-react/icon'
...
const Navbar: FunctionComponent<NavbarProps> = ({ children, className, ...props }) => {
  const id = useId()
...
const logo_cva = cva(['font-semibold max-w-[200px] font-heading text-txt-black-900'], {
  variants: {
    too_long: {
      false: 'text-body-lg',
...
const NavbarLogo: FunctionComponent<NavbarLogoProps> = ({
  ...
  return (
    <Link
      href={href}
      underline="none"
      className={cn('flex h-full w-auto items-center gap-2.5', className)}
      {...props}
    >
      <img
        src={src}
      ...
      {typeof children === 'string' ? (
        <span className={logo_cva({ too_long: children.length > maxChars })}>{children}</span>
      ) : (
        children
      )}
    </Link>
    ...
    <RemoveScroll as={Root} allowPinchZoom enabled>
      <Popover open={show} onOpenChange={setShow}>
        ...
        <PopoverContent
          sideOffset={0}
          align="start"
          className={cn('absolute top-full z-40 h-dvh xl:hidden')}
          style={{
...
interface NavbarMenuItemProps extends ComponentProps<typeof NavigationMenuItem> {
  children: ReactNode
  href: string
  className?: string
  asChild?: boolean
...
const NavbarMenuItem: FunctionComponent<NavbarMenuItemProps> = ({
  ...
  return (
    <NavigationMenuItem
      className={cn(navbar_menu_item_cva({ is_from_dropdown }), className)}
      {...props}
    >
...
export { Navbar, NavbarLogo, NavbarAction, NavbarMenu, NavbarMenuItem, NavbarMenuDropdown }
